# The prompts I ran (AOS example), verbatim

Every agent in the run received `SHARED_SPEC` (always) and, except the Contract agent,
`CONTRACT_BLOCK` (the prose contract the Contract agent returned). Then its own task. This file
reproduces each piece exactly as sent. The full orchestration script is
[`../../workflows/aos-sse-pipeline-refactor.workflow.js`](../../workflows/aos-sse-pipeline-refactor.workflow.js).

---

## SHARED_SPEC (prepended to every agent)

```
PROJECT: /Users/basemaloumar/Developer/AOS — TS/React 19 + Vite 6 + Express 4 + @google/genai ^2.4.0, tailwind v4, lucide-react, motion, react-markdown.
...
## Phase 1 — Contract agent (runs alone)
...
Its returned `contract` text is injected into every Phase-2+ prompt as `CONTRACT_BLOCK`.

---

## Phase 2 — Build, BACKEND agent (parallel with frontend)
...
## Phase 2 — Build, FRONTEND agent (parallel with backend)
...
## Phase 3 — Verify & repair agent (runs alone)

```
YOUR TASK (Phase 3 — VERIFY & REPAIR) in /Users/basemaloumar/Developer/AOS.
...
## Phase 4 — Adversarial SSE audit agent (read-only)
...
## Phase 4b — Conditional repair agent (only if criticals/majors found)

Dispatched only when the audit returns `severity ∈ {critical, major}`. The confirmed findings are
